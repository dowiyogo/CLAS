...
// how to iterate through the TTree.
  ...
  while ( thrownreaderD.Next() ) {
	...
	if ( pid_thrownD[i] == 8 && xf_thrownD[i] > 0.1 && thrown_energyD < 2.0 ) {
	  if ( xf_acceptD[j] > 0.1 && accept_energyD < 2.0 && abs(phipq_thrownD[i] - phipq_acceptD[j]) < 4.0 ) {
...
DaFe->Draw("Zh*Nu>>Fe_data()","Zh*Nu<2.0&&Xf>0.1"); //  del TNtuple como un Histograma
hist_dataFe = (TH1F *)gROOT->FindObject("Fe_data");
 ...
 TTreeReader datareaderFe("ntuple_data",dataFe);
  ...
  while ( thrownreaderFe.Next() ) {
	...
	if ( pid_thrownFe[i] == 8 && xf_thrownFe[i] > 0.1 && thrown_energyFe < 2.0 ) {
	  if ( xf_acceptFe[j] > 0.1 && accept_energyFe < 2.0 && abs(phipq_thrownFe[i] - phipq_acceptFe[j]) < 4.0 ) {
...
TCanvas* c7FeD= new TCanvas("c6FeD","Pion Energy D and Fe Deconvolution Normalized",10,10,700,900);
hRecoFe->DrawNormalized();
